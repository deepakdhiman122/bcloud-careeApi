...
const register_user = async (req, res) => {
  try {
    ...
    if (userData) {
      ...
      res
        .status(200)
        .send({ success: true, data: user_data, msg: "register successfully" });
    }
  } catch (error) {
    res.status(400).send(error.message);
    console.log(error.message);
  ...
  try {
    ...
    if (userData) {
      ...
      if (passwordMatch) {
        ...
        res
          .status(200)
          .send({ success: false, msg: "Login details are incorrected !" });
      }
    } else {
      res
        .status(200)
...
const get_register = async (req, res) => {
  try {
    // change user to Users 2/7/2024
    const articles = await Users.find();
